...
- **A running `xi_map` hot-reloads `scripts/`, `modules/` and `settings/` the moment a file is saved** (`filewatcher` in `luautils.cpp`; log lines `[FileWatcher] ...`, `RELOADING ALL LUA SETTINGS FILES`). Edits go live for connected testers immediately, so don't save a half-finished script while people are online. New module files and Module overrides are NOT applied by the watcher; they need a restart. Test files under `scripts/tests/` log a harmless `attempt to call global 'describe'` load error when saved, because only `xi_test` defines `describe`.
